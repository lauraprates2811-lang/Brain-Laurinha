# Vestibular

> **Status:** Inscrita e confirmada no Insper (11/10), na FGV (18–19/10) e no ENEM 2026.
> FGV é prioridade absoluta. Faltam **26 dias** para a 1ª fase. A 2ª fase é **Exame Oral** — peso 2,
> elimina, e só pode ser feito uma vez. O banco das provas 2021.1–2026.1 (423 itens) já diz o que mais cai.

## O que é

A frente principal. Três portas de entrada na FGV Direito SP, e a Laura pode usar mais de uma
(inscrição e taxa separadas para cada, item 9.2.c do edital). Se passar em mais de uma, só pode
matricular na que sair primeiro.

### As vagas da FGV Direito SP (132 de 160 autorizadas pelo MEC)

| Via | Vagas | Inscrição | Situação da Laura |
|---|---|---|---|
| Vestibular Regular | **105** | encerrada em 16/09 | ✅ inscrita |
| Vestibular DSD | 13 | encerrada em 16/09 | não se aplica |
| ENEM | **8** | até **08/01/2027** | inscrita no ENEM 2026; falta **inscrever-se nesta via da FGV** |
| IB | 2 | até 18/12/2026 | não tem IB Diploma (a EABH é IB só no PYP/MYP) |
| ABITUR | 1 | até 18/12/2026 | não se aplica |
| BAC | 1 | até 18/12/2026 | não se aplica |
| **SAT** | **2** | até **18/12/2026** | nunca fez SAT — **carta na manga**, ver abaixo |

### Estrutura da prova da FGV

**1ª fase — 18 e 19/10/2026**

| Quando | Provas |
|---|---|
| 18/10 manhã (08h–11h30) | Discursivas: **Redação** · **Ciências Humanas** (História e Geografia) |
| 18/10 tarde (14h30–18h) | Objetivas: **Matemática** · **Língua Portuguesa** · **Inglês** · **Ciências Humanas** (Atualidades, História, Geografia) |
| 19/10 manhã (08h–11h) | Discursivas: **Língua Portuguesa** · **Artes e Questões Contemporâneas** |

**Elimina automaticamente** quem tirar nota bruta abaixo de 3,0 em: Redação, Língua Portuguesa
discursiva, Língua Portuguesa objetiva ou Inglês. Nota zero em qualquer prova também elimina.

**O que o banco de provas mostra (2023.1–2026.1, formato atual)** — conferir com `/banca`
- Objetiva: 15 Matemática · 15 Português (7 delas de Literatura) · 15 Inglês · 15 Humanas
  (3 Atualidades, 6 História, 6 Geografia).
- Discursiva: 8 Humanas · 8 Português · 5 Artes · Redação. Grade: 100/75/50/25% (completa ou
  parcial × com ou sem desvio de redação).

**2ª fase — Exame Oral, 18/11 a 01/12/2026**

- Presencial em São Paulo, em português, duração máxima de **2h30**, **peso 2**.
- Dia e hora individuais, divulgados em 16/11. **Não há remarcação, em nenhuma hipótese.**
- Acesso fecha **15 min antes**; chegar com 60 min de antecedência. Chegar depois = eliminada.
- Não é permitido acompanhante no local.
- Nota bruta **≤ 3,0 elimina**.
- ⚠️ **Uma única chance nas três portas:** quem é convocada e não comparece tira zero e **não
  pode refazer** no processo do ENEM nem no Internacional.

**O que o oral avalia** (item 9.7.2.c, literal): capacidade de analisar situações complexas,
enfrentar desafios e construir respostas consistentes **à luz da sua trajetória biográfica**;
domínio da linguagem oral; capacidade analítica e crítica; habilidade de formular e defender
argumentos; capacidade de **idealizar e concretizar iniciativas**; lidar com adversidades e
pressão; qualidade da **contribuição e interação em grupo**; exercer liderança ou reconhecer
lideranças legítimas.

### A carta na manga do SAT

O resultado final sai em **07/12** e a inscrição pela via SAT vai até **18/12**. Ou seja: ela
saberá se passou **antes** do prazo fechar. Se não passar, dá para tentar as 2 vagas de SAT com
uma prova feita em novembro ou dezembro — o exame precisa ter sido feito nos últimos 2 anos e a
nota mínima para Direito SP é **1.200**. Decisão dela hoje: não priorizar o SAT agora, o foco é
a prova de outubro. **Correto — mas a data 18/12 não pode ser perdida.**

## Pendências

- [ ] Montar o plano dos 26 dias até a FGV (`/plano`), usando os números do `/banca`
- [ ] Começar o treino do Exame Oral **agora**, não em novembro (`/oral`)
- [ ] Destrinchar a trajetória biográfica em `Projetos/Vestibular/oral/` — é avaliada literalmente
- [ ] Confirmar datas do ENEM 2026 no site do INEP `[a confirmar]`
- [ ] Confirmar data e forma do resultado do Insper `[a confirmar]`
- [ ] Decidir até 18/12 se vai usar a via SAT
- [ ] Inscrever-se na via ENEM da FGV até 08/01/2027 (taxa separada)
- [ ] Separar documentos de bolsa **antes** de 16/11, porque a janela é curta (16/11–30/11)
- [ ] Conferir o gabarito definitivo de 2026.1 e atualizar o banco (hoje está o preliminar)

## Regras da área

- A FGV manda. Insper é segunda opção e não deve tomar tempo de preparação específica.
- Faltar ao oral é o pior erro possível: zera as três portas de uma vez.
- Toda data de edital entra no `Calendario.md` com a fonte, nunca de memória.

## Projeções

*(preenchido pelo `/sono`)*

## Histórico (últimas entradas)

| Data | O que aconteceu | Resultado |
|---|---|---|
| 2026-09-21 | Datas, vagas e regras extraídas do edital oficial da FGV; descoberto que o Vestibular Regular também tem Exame Oral | Frente mapeada · oral virou prioridade |
| 2026-09-22 | 6 provas antigas (2021.1–2026.1) viraram banco de 423 questões classificadas, com gabarito | `/banca` e `/discursiva` no ar |

## 🔗 Relacionados

[[Calendario]] · [[Certificacao]] · [[Cursinho]] · [[Laura]] · [[Vestibular-projeto]]
